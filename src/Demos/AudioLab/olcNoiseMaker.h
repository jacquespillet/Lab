...
public:
	static vector<string> Enumerate()
	{
		...
		for (int n = 0; n < nDeviceCount; n++)
			...
			{
				string str(woc.szPname);
				sDevices.push_back(str);
			}
		return sDevices;
	}

	void SetUserFunction(double(*func)(void *, double), void* objectPointer)
	{
		m_userFunction = func;
...
private:
	double(*m_userFunction)(void *, double);
	...
	static void CALLBACK waveOutProcWrap(HWAVEOUT hWaveOut, UINT uMsg, DWORD_PTR dwInstance, unsigned long dwParam1, unsigned long dwParam2)
	{
		olcNoiseMaker *instance = (olcNoiseMaker*)dwInstance;
		instance->waveOutProc(hWaveOut, uMsg, dwParam1, dwParam2);
	}
		...
		while (m_bReady)
		{
			...
			for (unsigned int n = 0; n < m_nBlockSamples; n++)
			{
				...
				if (m_userFunction == nullptr)
					...
					nNewSample = (T)(clip(m_userFunction(m_objectPointer, m_dGlobalTime), 1.0) * dMaxSample);
